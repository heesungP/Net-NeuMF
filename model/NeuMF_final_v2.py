...
class NeuMF:
    ...
    def __init__(self, user_num, item_num, mat_num, train_tf):

        latent_features = 8
        print("abcde#################")
        print("Matrix_num : {}".format(mat_num))
        print("Train_T_F : {}".format(train_tf))
        
        with open('embedding_matrix_8.pickle', 'rb') as f:
            embedding_matrix_8 = pickle.load(f)
        print(embedding_matrix_8.shape)
        
        with open('embedding_matrix_32.pickle', 'rb') as f:
            embedding_matrix_32 = pickle.load(f)
        print(embedding_matrix_32.shape)
        ...
        if mat_num == 8:
            ...
            gmf_item_embedding = Embedding(item_num, latent_features, weights=[embedding_matrix_8], input_length=item.shape[1], trainable=train_tf)(item)
            ...
            mlp_item_embedding = Embedding(item_num, 8, weights=[embedding_matrix_8], input_length=item.shape[1], trainable=train_tf)(item)
            mlp_item_embedding = Flatten()(mlp_item_embedding)
            mlp_item_embedding = Dense(units=32, activation='relu')(mlp_item_embedding)
            ...
            print(gmf_user_embedding)
            # Item embedding for GMF
            gmf_item_embedding = Embedding(item_num, 32, weights=[embedding_matrix_32], input_length=item.shape[1], trainable=train_tf)(item)
            gmf_item_embedding = Flatten()(gmf_item_embedding)
            gmf_item_embedding = Dense(units=16, activation='relu')(gmf_item_embedding)
            gmf_item_embedding = Dense(units=8, activation='relu')(gmf_item_embedding)
            print(gmf_item_embedding)
            # User embedding for MLP
            mlp_user_embedding = Embedding(user_num, 32, input_length=user.shape[1])(user)
            mlp_user_embedding = Flatten()(mlp_user_embedding)
            print(mlp_user_embedding)
            # Item embedding for MLP
            mlp_item_embedding = Embedding(item_num, 32, weights=[embedding_matrix_32], input_length=item.shape[1], trainable=train_tf)(item)
            mlp_item_embedding = Flatten()(mlp_item_embedding)
            print(mlp_item_embedding)        
