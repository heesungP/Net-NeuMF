...
class NeuMF:
    ...
    def __init__(self, user_num, item_num, emb_size, emb_arr_u, emb_arr_i):
        ...
        mlp_user_embedding = Embedding(user_num, emb_size, weights=[emb_arr_u], input_length=user.shape[1])(user)
        mlp_user_embedding = Flatten()(mlp_user_embedding)
        print(mlp_user_embedding)
        # Item embedding for MLP
        mlp_item_embedding = Embedding(item_num, emb_size, weights=[emb_arr_i], input_length=item.shape[1])(item)
